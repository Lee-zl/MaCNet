...
def get_sequence(seq,seq_home):
    result_home= opts['res_dir'] # you need creat result dirpath
    img_dir1=os.path.join(seq_home,seq,'v')
    img_dir2=os.path.join(seq_home,seq,'i')
    gt_path=os.path.join(seq_home,seq,'init.txt')
    img_list1=os.listdir(img_dir1)
    img_list1.sort()
    img_list1=[os.path.join(img_dir1,x) for x in img_list1 if os.path.splitext(x)[1] == '.png' or os.path.splitext(x)[1] == '.bmp']
    img_list2=os.listdir(img_dir2)
    img_list2.sort()
    img_list2=[os.path.join(img_dir2,x) for x in img_list2 if os.path.splitext(x)[1] == '.png' or os.path.splitext(x)[1] == '.bmp']
    ...
    result_path=os.path.join(result_dir,'MaC_'+seq+'.txt')
    return img_list1,img_list2,init_bbox,gt,result_path


def forward_samples(model, image1,image2, samples, out_layer='Fconv3'):
    model.eval()
...
def train(model, criterion, optimizer, pos_feats, neg_feats, maxiter, in_layer='Ffc4'):
    model.train()
...
def run_mdnet(img_list1, img_list2, init_bbox, gt=None, savefig_dir='', display=False):
    ...
    criterion = BCELoss()
    init_optimizer = set_optimizer_F(model, opts['lr_init'], opts['lr_mult'])
    update_optimizer = set_optimizer_F(model, opts['lr_update'], opts['lr_mult'])
    ...
    for i in range(1, len(img_list1)):
        ...
        sample_scores = forward_samples(model, image1, image2, samples, out_layer='Ffc6')
...
if __name__ == "__main__":
    ...
    list_path=os.path.join('/home/omnisky/project-code/MaCNet-loss/datasets/gtot.txt')
    ...
    for i in range(n_seq):
        ...
        seq=seqs[i]
        print("------------------- NO.%d : %s  -------------------" % (i, seq))
        if 'MaC_'+seq+'.txt' not in os.listdir(opts['res_dir']):
            np.random.seed(123)
            torch.manual_seed(456)
            torch.cuda.manual_seed(789)
        
            img_list1,img_list2,init_bbox,gt,result_path=get_sequence(seq,opts['data_dir'])
